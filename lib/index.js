...
const DEFAULT_CONFIG = {
  ...
  balanceRefreshSec: 15, // 余额自动刷新周期(秒):getState 过期即拉官方余额,越小越实时(建议≥5 以免官方接口限流)
}
...
function buildState(config, ledger) {
  ...
  return {
    ...
    config: {
      ...
      balanceRefreshSec: Number(config.balanceRefreshSec) || 15,
    },
  }
}
...
class ValleyMeterService extends TypertRemoteService {
  ...
  async getState() {
    ...
    const refreshSec = Math.max(5, Number(this.config.balanceRefreshSec) || 60)
...
export function apply(ctx) {
  ...
  try {
    const refreshSec = Math.max(5, Number(service.config?.balanceRefreshSec) || 60)
