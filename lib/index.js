...
const DEFAULT_CONFIG = {
  position: 'footer', // footer(侧边栏底部) | dock(输入框下方) | off
  peakEnabled: true,
...
function createService(ctx) {
  ...
  const refreshBalance = async (force = false) => {
    const stale = Date.now() - (ledger.balance?.fetchedAt ?? 0) > 60_000
    if (!force && !stale) return buildState(config, ledger)
    const result = await queryBalance(ctx, config.apiKeyEnv ?? 'DEEPSEEK_API_KEY')
    ledger = { ...ledger, balance: result }
    saveLedger(ledger)
    return buildState(config, ledger)
  }
...
export function apply(ctx) {
  ...
  ctx.provide('valleyMeter', service)
  ...
  // 启动后延迟拉一次余额(不阻塞)。
  try {
    const timer = setTimeout(() => { void service.refreshBalance(false).catch(() => {}) }, 2000)
