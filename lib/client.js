...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.vm-track{position:relative;height:2px;min-width:0;border-radius:999px;background:var(--dsw-alias-bg-layer-3,transparent)}',
      '.vm-seg{position:absolute;top:0;height:100%}',
      '.vm-seg-peak{background:var(--vm-peak)}',
      '.vm-seg-valley{background:var(--vm-valley)}',
      '.vm-block{position:absolute;top:50%;width:7px;height:7px;border-radius:2px;transform:translate(-50%,-50%);pointer-events:auto;z-index:0}',
      '.vm-past{position:absolute;top:-9px;left:0;height:calc(100% + 18px);width:0;background:rgba(6,8,14,.30);pointer-events:none;z-index:1;transition:width .5s ease;border-radius:999px 0 0 999px}',
      '.vm-marker{position:absolute;top:-6px;width:2px;height:calc(100% + 12px);background:var(--dsw-alias-bg-base,#fff);transform:translateX(-50%);transition:left .5s ease;z-index:2;border-radius:2px;animation:vm-breathe 2.4s ease-in-out infinite}',
    ...
    function tOf(props, fallbackLocale) {
      return props?.t || makeT(fallbackLocale || 'zh')
    }
    function ValleyCard(props) {
      var __st = null
      try { __st = props.useValley ? props.useValley(function(s){ return s }) : 'no-useValley' } catch(e) { __st = 'SEL-ERR:' + e.message }
      ...
      // 颜色深浅表示各小时消耗金额;白色指针=当前时刻,已过时段整体压暗。
      if (peak) {
          ...
          H('div', { className: 'vm-trackcol' },
            H('div', { className: 'vm-track' },
              ...
              hours ? hours.map((v, h) => {
                ...
                const op = v > 0 ? 0.22 + 0.78 * frac : 0.12
    ...
    async function apply(ctx) {
      ...
      // footer=sidebar.footer.action(scope:root,常驻);dock=conversation.input.dock(scope:session,有会话才渲染)。
      // 卡片挂载位置(gate 由组件内部按 config.position 决定)。
      // 注册方式与 dsh-cost-meter 完全一致:get(slots) + inject(slotName, cb)。
      const registerCard = (position) => {
        const slotName = position === 'footer' ? 'sidebar.footer.action' : 'conversation.input.dock'
        const slots = ctx.get('slots')
