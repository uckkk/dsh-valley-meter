...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 收起态:低调余额徽章(贴右);整枚可点,点击在新页签打开官方充值页。
      '.vm-chip{display:inline-flex;align-items:center;gap:8px;padding:3px 10px;margin-right:88px;border-radius:999px;background:var(--dsw-alias-bg-layer-2,transparent);border:1px solid rgba(255,255,255,.07);color:var(--dsw-alias-label-secondary,#cfd3d6);font-size:var(--dsw-font-s-14-font-size,14px);cursor:pointer;user-select:none;text-decoration:none;transition:border-color .2s ease,background .2s ease,color .2s ease}',
      '.vm-chip:hover{border-color:var(--dsw-alias-border-l2,rgba(255,255,255,.14));background:var(--dsw-alias-bg-layer-3,transparent)}',
      '.vm-chip:focus-visible{outline:2px solid var(--vm-valley);outline-offset:2px}',
      '.vm-chip:hover .vm-chip-value{color:var(--dsw-alias-label-primary,#e9eaec)}',
      // 外链提示箭头:悬停/聚焦时淡入,提示"点了会跳走"。
      '.vm-chip-arrow{font-size:10px;line-height:1;color:var(--dsw-alias-label-tertiary,#6a6e75);opacity:0;transform:translateX(-2px);transition:opacity .2s ease,transform .2s ease}',
      '.vm-chip:hover .vm-chip-arrow,.vm-chip:focus-visible .vm-chip-arrow{opacity:1;transform:translateX(0)}',
      '.vm-chip-label{font-size:var(--dsw-font-xs-13-font-size,13px);color:var(--dsw-alias-label-caption,#81858c)}',
      '.vm-chip-value{font-weight:600;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-secondary,#cfd3d6)}',
      '.vm-chip-caret{font-size:10px;color:var(--dsw-alias-label-tertiary,#6a6e75)}',
    ...
    function ValleyCard(props) {
      ...
      return H('div', { className: 'vm-root', ref: rootRef,
        ...
        // 余额 chip(默认只显示数字,低调);整枚是链接:点击在新页签打开官方充值页。
        // 用 <a target="_blank"> 而非 window.open:交给宿主/浏览器原生处理,
        // 桌面端会走外部浏览器打开,也自带键盘可达与"新页签"语义。
        H('a', {
          className: 'vm-chip',
          href: TOP_UP_URL,
          target: '_blank',
          ...
          H('span', { className: 'vm-chip-value' }, balanceVal),
          H('span', { className: 'vm-chip-arrow', 'aria-hidden': 'true' }, '↗'),
          H('span', { className: 'vm-chip-caret' }, open ? '▾' : '▴')))
    }
