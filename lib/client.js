...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.vm-cell-zone{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark;position:relative;display:flex;justify-content:flex-end;align-items:center;min-height:0;font-size:var(--dsw-font-s-14-font-size,14px);pointer-events:auto;padding:2px 4px;box-sizing:border-box}',
      ...
      // 悬停浮现的峰谷时间轴:铺在本 cell 顶部(top:0)= 紧贴输入框上沿,一条横线。
      '.vm-trackline{position:absolute;top:0;left:0;right:0;padding:2px 0;box-sizing:border-box;animation:vm-pop-in .2s ease;z-index:49}',
      // 面板头部:名称 + 极简设置齿轮
      ...
      // 设置浮层(点击齿轮弹出,不经过市场)
      '.vm-pop{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:min(560px,92vw);max-height:82vh;overflow:auto;padding:16px 18px;border-radius:16px;background:var(--dsw-alias-bg-layer-2,#1c1d21);border:1px solid rgba(255,255,255,.1);box-shadow:0 18px 60px rgba(0,0,0,.5);z-index:10000;box-sizing:border-box}',
      '.vm-pop-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}',
      '.vm-pop-title{font-size:var(--dsw-font-base-strong-16-font-size,16px);font-weight:650;color:var(--dsw-alias-label-primary,#e9eaec)}',
      '.vm-pop-close{background:none;border:none;padding:4px;font-size:15px;line-height:1;color:var(--dsw-alias-label-tertiary,#6a6e75);cursor:pointer;border-radius:6px;transition:color .2s ease,background .2s ease}',
      '.vm-pop-close:hover{color:var(--dsw-alias-label-primary,#e9eaec);background:rgba(255,255,255,.06)}',
      // 弹入动效
    ...
    function ValleyCard(props) {
      ...
      // 锚定到输入框:用 position:fixed 定位到输入框上方右侧,紧贴、不撑满。
      // 面板向上/向右在视口内展开,不受槽位布局与 overflow 裁剪影响。
      React.useEffect(() => {
        const el = rootRef.current
        if (!el) return undefined
        // 探测多种输入元素(textarea / contenteditable / [role=textbox]),
        // 拿不到时回退屏幕右下角,保证浮层始终可见可交互。
        const findInput = () => (document.querySelector('textarea'))
          || document.querySelector('[contenteditable="true"], [contenteditable=""], [role="textbox"]')
          || null
        const syncPos = () => {
          ...
          const chip = el.querySelector('.vm-chip')
          const chipW = chip?.offsetWidth || 120
          if (r && r.width > 0 && r.top > 0) {
            // 贴输入框右缘 + 输入框顶上方;
            el.style.left = Math.round(r.right) + 'px'
            el.style.bottom = Math.round(vh - r.top - 34) + 'px'
          } else {
            // 回退:屏幕右下角,给 chip 留空间。
            el.style.left = Math.round(vw - chipW - 16) + 'px'
        ...
        return () => { ro.disconnect(); window.removeEventListener('resize', syncPos) }
      }, [])
      ...
      // conversation.input.dock 的一个 cell(order 30,排在 todo/goal/queue 之后靠右):
      //   - 默认:只在右侧(红框)显示余额 chip,低调;
      //   - 悬停:chip 上方浮现峰谷时间轴(紧贴 cell 顶=输入框上沿)+ 展开面板(余额/今天/倒计时)。
      // 纯自然布局,不做 fixed 手工坐标。
      return H('div', { className: 'vm-cell-zone', ref: rootRef },
        // 峰谷时间轴(蓝线):悬停该 zone 才浮现,absolute 锚在 cell 顶部(输入框上沿)。
        ...
        H('div', {
          ...
          // 展开面板:chip 附近弹出;含余额/今天/倒计时 + 设置按钮
          open ? H('div', {
            className: 'vm-panel',
            onMouseEnter: () => setOpen(true),
          },
            H('div', { className: 'vm-panel-head' },
              H('span', { className: 'vm-panel-title' }, t('dockTitle')),
              H('button', {
                className: 'vm-gear',
                title: t('settings'),
                'aria-label': t('settings'),
                onClick: (e) => { e.stopPropagation(); setOpen(false); setShowSettings(true) },
              }, '⚙')),
        ...
        // 设置浮层(点击齿轮弹出,不经过市场);独立 fixed 居中,与面板互斥
        showSettings ? H('div', { className: 'vm-pop', onClick: (e) => e.stopPropagation() },
          H('div', { className: 'vm-pop-head' },
            H('span', { className: 'vm-pop-title' }, t('settings')),
            H('button', { className: 'vm-pop-close', onClick: () => setShowSettings(false) }, '✕')),
          H(ValleySettings, { ...props })) : null)
    }
    ...
    async function apply(ctx) {
      ...
      // 卡片挂到输入框上方整行(conversation.input.dock,scope:session)——
      // 与 todo/goal/queue 同排(todo@0 goal@10 queue@20,本插件 order 30 在最右)。
      // 利用整行布局:右侧(红框)常驻余额 chip,行上沿(蓝线)悬浮才浮现峰谷时间轴,
      // 时间轴自然紧贴输入框上沿,无需手工坐标定位。
      const registerCard = () => {
