...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.vm-trackline{width:min(300px,64%);align-self:flex-end;margin-top:2px;box-sizing:border-box;animation:vm-pop-in .2s ease}',
    ...
    function ValleyCard(props) {
      ...
      // vm-root 由 effect 锚定到输入框卡片右上角上方(fixed,底部贴卡片上沿,右缘贴卡片右缘)。
      // chip 常驻靠右;时间轴悬停时在 chip 下方(贴输入框上沿)浮现;面板在 chip 上方弹出。
      return H('div', { className: 'vm-root', ref: rootRef },
        ...
        H('div', {
          ...
          open ? panel : null),
        // 峰谷时间轴:悬停浮现,贴在 chip 下方(紧贴输入框上沿),短、圆角、靠右。
