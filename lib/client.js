...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.vm-card{display:flex;flex-direction:column;gap:10px;padding:12px 14px;border-radius:16px;background:var(--dsw-alias-bg-layer-2,transparent);border:1px solid var(--dsw-alias-border-l2,transparent);min-width:260px;box-shadow:0 1px 3px rgba(0,0,0,.08)}',
      ...
      '.vm-track{position:relative;display:flex;flex:1;height:8px;min-width:0;border-radius:999px;overflow:hidden;border:1px solid var(--dsw-alias-border-l1,transparent);background:var(--dsw-alias-bg-layer-3,transparent)}',
      '.vm-seg{position:absolute;top:0;height:100%}',
      '.vm-seg-peak{background:var(--vm-peak)}',
      '.vm-seg-valley{background:var(--vm-valley)}',
      '.vm-marker{position:absolute;top:-2px;width:3px;height:calc(100% + 4px);background:var(--dsw-alias-bg-base,#fff);box-shadow:0 0 0 1px var(--dsw-alias-label-tertiary,rgba(0,0,0,.35));transform:translateX(-50%);transition:left .5s ease;z-index:2;border-radius:2px}',
      ...
      if (peak) {
        children.push(H('div', { className: 'vm-strip' },
          H('div', { className: 'vm-track' },
            ...
            dayPeakSegments(peak.windows).map((s, i) => H('div', { key: i, className: 'vm-seg vm-seg-peak', style: { left: s.left, width: s.width } })),
            H('div', { className: 'vm-marker', style: { left: peakProgress(peak, now) } })),
