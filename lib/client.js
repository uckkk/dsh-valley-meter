...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.vm-trackline{position:fixed;top:50%;right:8px;transform:translateY(-50%);box-sizing:border-box;animation:vm-pop-in .2s ease;z-index:49}',
      // 竖向时间轴:细长竖条(峰橙/谷蓝),刻度在轴右侧。
      '.vm-rail{display:flex;align-items:stretch;gap:6px}',
      '.vm-rail-track{position:relative;width:5px;height:180px;border-radius:999px;background:var(--dsw-alias-bg-layer-3,transparent);box-shadow:inset 0 0 0 1px rgba(255,255,255,.08);overflow:hidden}',
      '.vm-rail-seg{position:absolute;left:0;width:100%}',
      '.vm-rail-peak{background:var(--vm-peak)}',
      '.vm-rail-valley{background:var(--vm-valley)}',
      '.vm-rail-marker{position:absolute;left:-3px;width:11px;height:2px;background:var(--dsw-alias-label-secondary,#cfd3d6);transform:translateY(-50%);transition:top .5s ease;z-index:2;border-radius:2px;animation:vm-breathe 2.4s ease-in-out infinite}',
      '.vm-rail-ticks{position:relative;width:18px;font-size:10px;line-height:1;color:var(--dsw-alias-label-caption,#81858c);font-variant-numeric:tabular-nums}',
      '.vm-rail-ticks span{position:absolute;right:0;transform:translateY(-50%)}',
      // 面板头部:名称 + 极简设置齿轮
    ...
    function ValleyCard(props) {
      ...
      // 倒计时/时间轴只在展开面板内渲染。
      if (countdown) {
        detailNodes.push(H('div', { className: 'vm-top' },
          H('span', { className: 'vm-countdown' }, countdown),
          H('span', { className: 'vm-cd-tag' }, peak.nextIntoPeak ? t('cdAfterPeak') : t('cdAfterValley'))))
      }
      // 峰谷时间轴(竖向):细长竖条表达峰(橙)/谷(蓝)时段,自上而下=00→24 时;
      // 当前时刻用横向 marker 标记,时间刻度在轴右侧竖向分布。
      let stripNode = null
      if (peak) {
        const prog = peakProgress(peak, now)
        stripNode = H('div', { className: 'vm-rail' },
          H('div', { className: 'vm-rail-track' },
            H('div', { className: 'vm-rail-seg vm-rail-valley', style: { top: '0%', height: '100%' } }),
            dayPeakSegments(peak.windows).map((s, i) => H('div', { key: 's' + i, className: 'vm-rail-seg vm-rail-peak', style: { top: s.top, height: s.height } })),
            H('div', { className: 'vm-rail-marker', style: { top: prog } })),
          H('div', { className: 'vm-rail-ticks' },
            ['00', '06', '12', '18', '24'].map(t => H('span', { key: t, style: { top: `${(Number(t) / 24 * 100).toFixed(1)}%` } }, t))))
      ...
      const panel = H('div', { className: 'vm-panel', onMouseEnter: () => setOpen(true) },
        H('div', { className: 'vm-panel-head' },
          H('span', { className: 'vm-panel-title' }, showSettings ? t('settings') : t('dockTitle')),
          showSettings
      ...
      // chip 作为 hover 触发器,默认只显示余额数字;panel/trackline 是 vm-root 的 absolute 子元素,
      // 悬停时在 chip 上方(面板)与下方(时间轴,贴输入框上沿)浮现。
      return H('div', { className: 'vm-root', ref: rootRef,
        onMouseEnter: () => setOpen(true),
        onMouseLeave: () => { if (!showSettings) setOpen(false) },
        ...
        // 展开面板(悬停浮现,在 chip 上方弹出)
        open ? panel : null,
        // 峰谷时间轴(悬停浮现,贴输入框上沿,短线圆角)
    ...
    // 把 UTC 峰时段窗口映射成竖向 24h 时间轴的绝对位置(%字符串, top=开始, height=时长)。
    function dayPeakSegments(windows) {
      ...
      for (const w of windows) {
        ...
        for (const [s, e] of ranges) {
          segs.push({ top: `${(s / 24 * 100).toFixed(2)}%`, height: `${((e - s) / 24 * 100).toFixed(2)}%` })
        }
      }
      return segs
    }
    ...
    async function apply(ctx) {
      ...
      // 不需要手工坐标定位,也不会像 shell.overlay 那样漂移)。cell 设固定最小高度,
      // 即使无 todo/goal/queue 也撑起一行,始终显示。
      const registerCard = () => {
        ...
        try {
          return slots.inject(slotName, () => {
            return slots.register(
              { name: slotName, id: 'valley-meter', order: 30, inject: injected }, ValleyCard)
          })
        } catch (e) {
