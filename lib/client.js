  ...
  factory: (require) => {
    ...
    function ValleyCard(props) {
      ...
      React.useEffect(() => {
        ...
        const inputCard = ta ? ta.closest('form, [data-composer], [role="textbox"]') || ta.parentElement : null
        const syncPos = () => {
          const r = (inputCard || ta)?.getBoundingClientRect()
          if (!r || r.width === 0) return
          // 贴输入框右缘,上边紧贴输入框顶(底部留出 chip 高度)。
        ...
        if (inputCard) ro.observe(inputCard)
        if (ta) ro.observe(ta)
