...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 详情默认收起,光标悬停/聚焦卡片区时展开完整信息(倒计时/时间轴/时段徽标)
      '.vm-fade{opacity:0;max-height:0;overflow:hidden;margin-top:0;transition:max-height .3s ease,opacity .3s ease,margin-top .3s ease}',
      '.vm-card.vm-open .vm-fade{opacity:1;max-height:120px;margin-top:0}',
      '@media (hover:none){.vm-fade{opacity:1;max-height:120px;margin-top:0}}',
    ...
    function ValleyCard(props) {
      ...
      // 详情默认收起,光标悬停卡片区才展开完整信息(倒计时/时间轴/时段徽标)。
      ...
      if (countdown) {
        children.push(H('div', { className: 'vm-top vm-fade' },
      ...
      if (peak) {
        ...
        children.push(H('div', { className: 'vm-strip vm-fade' },
      ...
      // 卡片内部:常驻「余额/今日」两栏在上,详情(倒计时/时间轴/时段)收起、悬停展开。
      return H('div', { className: 'vm-root', ref: rootRef },
        H('div', {
          className: 'vm-card ' + styleCls + (open ? ' vm-open' : ''),
