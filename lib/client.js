...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.vm-root{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark;position:fixed;z-index:900;display:flex;flex-direction:column;align-items:flex-end;font-size:var(--dsw-font-s-14-font-size,14px);pointer-events:auto;box-sizing:border-box}',
    ...
    function ValleyCard(props) {
      ...
      // 锚定到输入框卡片:用 position:fixed 定位到卡片右上角上方。
      // 输入框是 Lexical 的 contenteditable 编辑器(非 textarea),往上找圆角≥16 的
      // 祖先即 composer 卡片(krUYjW_card, border-radius:22px)。
      React.useEffect(() => {
        ...
        const syncPos = () => {
          ...
          if (r && r.width > 0 && r.top > 0) {
            // chip 底部紧贴卡片上沿,右缘对齐卡片右缘(留 2px)。
            el.style.right = Math.max(0, Math.round(vw - r.right + 2)) + 'px'
            el.style.bottom = Math.max(0, Math.round(vh - r.top)) + 'px'
          } else {
            // 回退:屏幕右下角上方。
            el.style.right = '16px'
