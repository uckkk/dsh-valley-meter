...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 展开面板:绝对定位在 chip 上方(向上弹出),默认隐藏,悬挂 .vm-open 显示
      '.vm-panel{position:absolute;bottom:calc(100% + 8px);right:0;width:min(360px,90vw);padding:12px 14px;border-radius:16px;background:var(--dsw-alias-bg-layer-2,#1c1d21);border:1px solid rgba(255,255,255,.08);box-shadow:0 8px 30px rgba(0,0,0,.35);display:flex;flex-direction:column;gap:10px;box-sizing:border-box;opacity:0;visibility:hidden;transform:translateY(8px);transition:opacity .22s ease,transform .22s ease,visibility .22s ease;z-index:50;pointer-events:none}',
      '.vm-dock.vm-open .vm-panel{opacity:1;visibility:visible;transform:translateY(0);pointer-events:auto}',
      // 面板头部:名称 + 极简设置齿轮
      ...
      // 详情元素默认随面板一起,通过 vm-open 控制(去掉旧的 vm-fade 独立逻辑)
      '@media (hover:none){.vm-dock .vm-panel{opacity:1;visibility:visible;transform:translateY(0);pointer-events:auto}}',
      // 窄容器适配:藏徽标、减刻度、缩倒计时
    ...
    function ValleyCard(props) {
      ...
      return H('div', { className: 'vm-root', ref: rootRef },
        H('div', {
          className: 'vm-dock ' + (open ? 'vm-open' : ''),
          onMouseEnter: () => setOpen(true),
          onMouseLeave: () => setOpen(false),
          onFocus: () => setOpen(true),
          onBlur: () => setOpen(false),
          onClick: () => { if (open && api?.refreshBalance) {
            // 展开态点击余额不动;这里保留给隐藏 toggle 的语义,不改变现有行为。
          } },
        },
          ...
          // 展开面板(向上展开,含完整信息);用一个兄弟节点在 chip 下方叠出
          H('div', { className: 'vm-panel' },
            // 面板头部:标题 + 极简设置按钮
    ...
    async function apply(ctx) {
      ...
      // 卡片挂在输入框上方(conversation.input.dock,scope:session),低调贴右的余额徽章;
      // 光标悬停向上展开完整信息,展开后右上角有设置按钮点击弹出设置浮层。
      const registerCard = () => {
