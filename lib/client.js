...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.vm-trackline{position:absolute;top:100%;right:0;width:min(300px,64%);margin-top:2px;box-sizing:border-box;animation:vm-pop-in .2s ease;z-index:49}',
    ...
    function ValleyCard(props) {
      ...
      // 锚定到输入框卡片:vm-root 是 absolute,相对 shell.overlay 层(inset:0 铺满 frame)。
      // 输入框是 Lexical 的 contenteditable 编辑器,往上找圆角≥16 的祖先即 composer 卡片。
      React.useEffect(() => {
        const el = rootRef.current
        if (!el) return undefined
        const overlay = document.querySelector('[data-shell-overlay]')
        const findCard = () => {
          // 优先 DSH 的稳定选择器: data-composer-input(输入框), 往上找卡片(krUYjW_card, r=22px)。
          ...
          for (let i = 0; node && i < 8; i++) {
            const br = parseFloat(getComputedStyle(node).borderRadius)
            if (br >= 16) return node
        ...
        const syncPos = () => {
          ...
          if (r && r.width > 0 && r.top > 0 && base) {
            // 相对 overlay 层:chip 底部贴卡片上沿,右缘对齐卡片右缘。
            el.style.right = Math.max(0, Math.round(base.right - r.right + 2)) + 'px'
