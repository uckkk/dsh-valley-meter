  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        positionOff: '关闭',
        hint: '提示',
        priceNote: '谷价 / 峰价取自 cost-meter,此处只展示倒计时与余额。',
        noData: '暂无数据',
        refresh: '已加载',
      },
      en: {
        ...
        positionOff: 'Off',
        hint: 'Hint',
        priceNote: 'Peak/valley pricing comes from cost-meter; this widget shows countdown & balance.',
        noData: 'No data',
        refresh: 'Loaded',
      },
    }
    ...
    const CSS = [
      '.vm-root{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark}',
      '.vm-card{display:flex;flex-direction:column;gap:10px;padding:12px 14px;border-radius:16px;background:var(--dsw-alias-bg-layer-2,transparent);border:1px solid var(--dsw-alias-border-l2,transparent);min-width:260px;box-shadow:0 1px 3px rgba(0,0,0,.08);container-type:inline-size}',
      '.vm-top{display:flex;align-items:center;justify-content:center;gap:8px;font-variant-numeric:tabular-nums}',
      '.vm-pin{font-size:13px;line-height:1;opacity:.75}',
      '.vm-countdown{font-size:26px;font-weight:650;letter-spacing:.5px;color:var(--dsw-alias-label-primary,inherit)}',
      '.vm-cd-tag{font-size:12px;color:var(--dsw-alias-label-secondary,inherit)}',
      ...
      '.vm-track{position:relative;display:flex;flex:1;height:10px;min-width:0;border-radius:999px;overflow:hidden;border:1px solid var(--dsw-alias-border-l1,transparent);background:var(--dsw-alias-bg-layer-3,transparent)}',
      '.vm-seg{position:absolute;top:0;height:100%}',
      '.vm-seg-peak{background:linear-gradient(180deg,color-mix(in srgb,var(--vm-peak),white 16%),var(--vm-peak));box-shadow:inset 0 1px 0 rgba(255,255,255,.28)}',
      '.vm-seg-valley{background:linear-gradient(180deg,color-mix(in srgb,var(--vm-valley),white 14%),var(--vm-valley));box-shadow:inset 0 1px 0 rgba(255,255,255,.22)}',
      '.vm-seg-active::after{content:"";position:absolute;inset:0;border-radius:inherit;background-image:linear-gradient(105deg,transparent 32%,rgba(255,255,255,.4) 50%,transparent 68%);background-size:220% 100%;background-repeat:no-repeat;animation:vm-shimmer 3.2s linear infinite}',
      '.vm-past{position:absolute;left:0;top:0;height:100%;width:0;background:rgba(6,8,14,.34);pointer-events:none;z-index:1;transition:width .5s ease}',
      '.vm-marker{position:absolute;top:-2px;width:3px;height:calc(100% + 4px);background:var(--dsw-alias-bg-base,#fff);transform:translateX(-50%);transition:left .5s ease;z-index:2;border-radius:2px;animation:vm-breathe 2.4s ease-in-out infinite}',
      '.vm-ticks{display:flex;justify-content:space-between;margin-top:4px;font-size:9px;line-height:1;color:var(--dsw-alias-label-tertiary,inherit);font-variant-numeric:tabular-nums;padding:0 1px}',
      ...
      '.vm-bottom{display:grid;grid-template-columns:1fr 1fr;gap:10px}',
      '.vm-cell{display:flex;flex-direction:column;gap:2px;min-width:0}',
      '.vm-cell-title{font-size:12px;color:var(--dsw-alias-label-secondary,inherit);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
      '.vm-cell-value{font-size:20px;font-weight:650;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,inherit);white-space:nowrap}',
      '.vm-badge{display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:650;padding:2px 8px;border-radius:999px;border:1px solid var(--dsw-alias-border-l1,transparent);color:var(--dsw-alias-label-secondary,inherit);align-self:center;margin-left:auto}',
      ...
      // 极简样式
      '.vm-card.vm-style-minimal{gap:6px;padding:9px 12px;min-width:150px}',
      '.vm-card.vm-style-minimal .vm-countdown{font-size:20px}',
      '.vm-card.vm-style-minimal .vm-bottom{gap:14px}',
      '.vm-card.vm-style-minimal .vm-cell{flex-direction:row;align-items:baseline;gap:4px}',
      '.vm-card.vm-style-minimal .vm-cell-value{font-size:17px}',
      // 设置页
      ...
      // 峰段起止刻度(容器宽度足够时显示)
      '.vm-range{position:absolute;inset:0;display:none;align-items:center;justify-content:center;font-size:9px;font-weight:650;letter-spacing:.4px;color:rgba(20,12,0,.58);pointer-events:none;font-variant-numeric:tabular-nums;white-space:nowrap}',
      '@container (min-width:400px){.vm-seg-peak .vm-range{display:flex}}',
      // 动效
      '@keyframes vm-breathe{0%,100%{box-shadow:0 0 0 1px var(--dsw-alias-label-tertiary,rgba(0,0,0,.35)),0 0 5px 0 rgba(255,255,255,.16)}50%{box-shadow:0 0 0 1px var(--dsw-alias-label-tertiary,rgba(0,0,0,.35)),0 0 11px 2px rgba(255,255,255,.42)}}',
      '@keyframes vm-shimmer{from{background-position:135% 0}to{background-position:-135% 0}}',
      '@media (prefers-reduced-motion:reduce){.vm-marker{animation:none}.vm-seg-active::after{animation:none}}',
    ]
    ...
    function ValleyCard(props) {
      ...
      const api = props?.hooks?.valley?.api
      const t = props?.hooks?.valley?.t || makeT('zh')
      if (!state) return null

      const { config } = state
      const now = useTick(config?.showCountdown ? 1000 : 60000)
      ...
      // 顶部:倒计时(可选)
      if (countdown && config?.showCountdown !== false) {
        children.push(H('div', { className: 'vm-top' },
          H('span', { className: 'vm-pin' }, '📍'),
          H('span', { className: 'vm-countdown' }, countdown)))
      }

      // 峰谷进度条(按 24h 真实时段绘制峰段位置,白色指针=当前时刻,
      // 已过时段压暗,当前所处峰段流光高亮,下方 00–24 时间刻度)
      if (peak) {
        const d = new Date(now)
        const curH = d.getUTCHours() + d.getUTCMinutes() / 60 + d.getUTCSeconds() / 3600
        ...
        children.push(H('div', { className: 'vm-strip' },
          H('div', { className: 'vm-trackcol' },
            H('div', { className: 'vm-track' },
              ...
              dayPeakSegments(peak.windows, curH).map((s, i) => H('div', {
                key: i,
                className: 'vm-seg vm-seg-peak' + (s.active ? ' vm-seg-active' : ''),
                style: { left: s.left, width: s.width },
              }, H('span', { className: 'vm-range' }, s.label))),
          ...
          H('span', { className: 'vm-hh' }, clockHm(now))))
      }

      if (peak && config?.showPeriod !== false) {
        children.push(H('div', { className: 'vm-legend' },
      ...
      children.push(H('div', { className: 'vm-bottom' },
      ...
      return H('div', { className: 'vm-root' }, H('div', { className: 'vm-card ' + styleCls }, ...children))
    }
    ...
    function ValleySettings(props) {
      ...
      const color = (key) => (e) => update({ [key]: e.target.value })
      ...
      return H('div', { className: 'vm-set' },
        H('div', { className: 'vm-set-row-group' },
          H('div', { className: 'vm-set-row' },
            ...
            H('input', { type: 'color', className: 'vm-set-color', value: cfg.valleyColor || '#3b6ef5', onChange: color('valleyColor') })),
          H('div', { className: 'vm-set-row' },
            H('label', { className: 'vm-set-label' }, t('peakColor')),
            H('input', { type: 'color', className: 'vm-set-color', value: cfg.peakColor || '#f5941b', onChange: color('peakColor') }))),

        H('div', { className: 'vm-set-row' },
          H('label', { className: 'vm-set-label' }, t('style')),
          H('select', { className: 'vm-set-input', value: cfg.style || 'card', onChange: setS('style') },
            styleOpts.map(o => H('option', { key: o.v, value: o.v }, o.label)))),

        H('div', { className: 'vm-set-row' },
          H('label', { className: 'vm-set-label' }, t('position')),
          H('select', { className: 'vm-set-input', value: cfg.position || 'dock', onChange: setS('position') },
    ...
    // 把 UTC 峰时段窗口映射成 24h 进度条的绝对位置(%字符串)。
    // 附带起止标签与"当前是否处于该峰段"(curH = UTC 小时浮点数)。
    function dayPeakSegments(windows, curH) {
      const segs = []
      if (!Array.isArray(windows)) return segs
      const inWin = (w, h) => w.start < w.end ? (h >= w.start && h < w.end) : (h >= w.start || h < w.end)
      for (const w of windows) {
        ...
        for (const [s, e] of ranges) {
          segs.push({
            left: `${(s / 24 * 100).toFixed(2)}%`,
            width: `${((e - s) / 24 * 100).toFixed(2)}%`,
            label: `${String(Math.floor(start)).padStart(2, '0')}–${String(Math.floor(end)).padStart(2, '0')}`,
            active: Number.isFinite(curH) && inWin({ start, end }, curH),
          })
        }
      }
      return segs
    ...
    async function apply(ctx) {
      ...
      const injected = () => ({ hooks: { valley: store }, api, t: makeT('zh') })
      ...
      const registerCard = (position) => {
        if (position === 'dock') {
          return slots.inject('conversation.composer.dock', () => slots.register(
            { name: 'conversation.composer.dock', id: 'valley-meter', order: 5, inject: injected }, ValleyCard))
        }
        if (position === 'footer') {
          return slots.inject('sidebar.footer.action', () => slots.register(
            { name: 'sidebar.footer.action', id: 'valley-meter', order: 0, inject: injected }, ValleyCard))
        }
        return () => {}
      }
