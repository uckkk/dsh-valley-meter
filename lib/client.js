...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.vm-root{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark;position:fixed;z-index:900;display:flex;flex-direction:column;align-items:flex-end;font-size:var(--dsw-font-s-14-font-size,14px);pointer-events:auto}',
      ...
      '.vm-panel{position:absolute;bottom:calc(100% + 8px);right:0;width:max-content;max-width:min(360px,90vw);padding:12px 14px;border-radius:16px;background:var(--dsw-alias-bg-layer-2,#1c1d21);border:1px solid rgba(255,255,255,.08);box-shadow:0 8px 30px rgba(0,0,0,.35);display:flex;flex-direction:column;gap:10px;box-sizing:border-box;animation:vm-pop-in .22s ease;z-index:50}',
      // 面板头部:名称 + 极简设置齿轮
    ...
    function ValleyCard(props) {
      ...
      // 收起态:一个贴右的低调余额徽章(只有余额,不含标题/今天花费)。
      // 悬浮/聚焦向上展开完整面板;展开后右上角设置按钮弹出设置浮层。
      // vm-root 由 effect 锚定到输入框上方右侧(position:fixed,右下基准),贴右、不撑满。
      // 面板在 vm-root 内 absolute:右下基准,向上(bottom:100%)向右(right:0)自动匹配宽度。
      return H('div', { className: 'vm-root', ref: rootRef },
        // 常驻:峰谷颜色变量
        ...children,
        // 收起态:极简余额徽章(贴右,只显示数值,低调)
        H('div', {
          className: 'vm-chip',
    ...
    async function apply(ctx) {
      ...
      // 卡片纳入帧级浮层(shell.overlay),组件内部用 position:fixed 锚定到输入框
      // 上方右侧——紧贴输入框、不撑满、悬浮向上向右自动匹配内容。
      const registerCard = () => {
