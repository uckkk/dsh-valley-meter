...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.vm-root{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark;position:absolute;z-index:900;right:16px;bottom:140px;display:flex;flex-direction:column;align-items:flex-end;font-size:var(--dsw-font-s-14-font-size,14px);pointer-events:auto;box-sizing:border-box}',
      ...
      '.vm-trackline{position:absolute;top:100%;right:0;width:var(--vm-track-w,min(300px,64%));margin-top:2px;box-sizing:border-box;animation:vm-pop-in .2s ease;z-index:49}',
    ...
    function ValleyCard(props) {
      ...
      // 锚定到输入框卡片:vm-root 是 absolute,相对 shell.overlay 层。
      // 用 DSH 稳定选择器 [data-composer-card] 定位整张卡片。
      // chip 右缘对齐卡片右上角后往左移一个 chip 宽(跟复制按钮水平对齐);
      // 垂直跟随卡片顶部(输入框变高时 chip 自动上移)。
      React.useEffect(() => {
        const el = rootRef.current
        if (!el) return undefined
        const overlay = document.querySelector('[data-shell-overlay]')
        const findCard = () => document.querySelector('[data-composer-card]')
        ...
        const syncPos = () => {
          ...
          const base = ov && ov.width > 0 ? ov : null
          const chip = el.querySelector('.vm-chip')
          const chipW = chip?.offsetWidth || 120
          if (r && r.width > 0 && r.top > 0 && base) {
            // 贴卡片右上角,再往左移一个 chip 宽(与左上复制按钮水平对应)。
            el.style.right = Math.max(0, Math.round(base.right - r.right + 2 + chipW)) + 'px'
            // chip 顶部贴卡片上沿(随卡片上移)。
            el.style.bottom = Math.max(0, Math.round(base.bottom - r.top)) + 'px'
            // 时间轴宽度:比卡片略短(卡片宽 - 24)。
            el.style.setProperty('--vm-track-w', Math.round(r.width - 24) + 'px')
          } else if (base) {
            el.style.right = '16px'
            el.style.bottom = '140px'
            el.style.setProperty('--vm-track-w', '300px')
          }
          el.style.left = 'auto'
          el.style.top = 'auto'
        }
    ...
    async function apply(ctx) {
      ...
      // 卡片纳入帧级浮层(shell.overlay,scope:root)始终渲染,不随 todo/goal 塌陷。
      // 组件内部用 position:fixed 锚定到输入框(contenteditable 编辑器)上方右侧。
      const registerCard = () => {
