...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.vm-cell-zone{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark;position:relative;display:flex;justify-content:flex-start;align-items:flex-start;min-height:0;font-size:var(--dsw-font-s-14-font-size,14px);pointer-events:auto;padding:2px 4px;box-sizing:border-box}',
      ...
      // 悬停浮现的峰谷时间轴:贴本 cell 底部(紧贴输入框上沿),短线、圆角,置于 chip 上方。
      '.vm-trackline{position:absolute;left:0;bottom:100%;width:min(60%,360px);padding:0 0 6px;box-sizing:border-box;animation:vm-pop-in .2s ease;z-index:49}',
      // 时间轴主体:圆角短线紧贴输入框上沿;刻度(0-24)放在轴上方。
      '.vm-trackline .vm-strip{width:100%}',
      '.vm-trackline .vm-track{border-radius:6px}',
      '.vm-trackline .vm-ticks{order:-1;margin:0 0 3px}',
      // 面板头部:名称 + 极简设置齿轮
