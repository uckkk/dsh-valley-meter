...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.vm-root{--vm-peak:#f5941b;--vm-valley:#3b6ef5;color-scheme:dark;width:100%;align-self:stretch}',
      '.vm-card{display:flex;flex-direction:column;gap:10px;padding:12px 14px;border-radius:16px;background:var(--dsw-alias-bg-base,#151517);border:1px solid rgba(255,255,255,.06);width:100%;max-width:100%;align-self:stretch;box-sizing:border-box;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);container-type:inline-size}',
      ...
      '.vm-marker{position:absolute;top:-5px;width:2px;height:calc(100% + 10px);background:var(--dsw-alias-label-secondary,#cfd3d6);transform:translateX(-50%);transition:left .5s ease;z-index:2;border-radius:2px;box-shadow:0 0 4px rgba(207,211,214,.35);animation:vm-breathe 2.4s ease-in-out infinite}',
      ...
      '.vm-cell-value{font-size:var(--dsw-font-base-16-font-size,16px);line-height:var(--dsw-font-base-16-line-height,24px);font-weight:500;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-secondary,#cfd3d6);white-space:nowrap}',
      ...
      // 倒计时/徽标默认淡出,悬停时间轴时浮现
      '.vm-fade{opacity:0;transition:opacity .3s ease}',
      '.vm-card:has(.vm-strip:hover) .vm-fade,.vm-card:has(.vm-trackcol:hover) .vm-fade{opacity:1}',
      '@media (hover:none){.vm-fade{opacity:1}}',
